...
export default function AdminUsersPage() {
  ...
  const columns = [
    ...
    { key: 'created_at', label: 'Joined', render: (u) => new Date(u.created_at).toLocaleDateString() },
    {
      key: 'actions', label: '',
      render: (u) => (
        <Link href={`/${locale}/admin/users/${u.id}`} className="text-gold hover:underline text-xs">
          View
        </Link>
      ),
    },
  ...
  return (
    <div>
      <h1 className="text-2xl font-light text-ocean mb-6">Users</h1>
      ...
      <DataTable
        ...
        onSearch={(q, p) => fetchUsers(q || search, p)}
      />
    </div>
  );
}
