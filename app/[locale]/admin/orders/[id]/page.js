...
export default function OrderDetailPage({ params: { id, locale } }) {
  ...
  return (
    <div>
      <Link href={`/${locale}/admin/orders`} className="text-sm text-gold hover:underline mb-4 inline-block">← Back to Orders</Link>
      <h1 className="text-2xl font-light text-ocean mb-6">Order #{order.id.slice(0, 8)}</h1>

      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-10">
        <div className="border border-zinc-200 rounded-xl p-6">
          <h2 className="font-medium text-ocean mb-4">Order Info</h2>
          <dl className="space-y-3 text-sm">
            <div className="flex items-center gap-2">
              <dt className="w-24 text-zinc-400">Status</dt>
            ...
            <div className="flex"><dt className="w-24 text-zinc-400">Date</dt><dd>{new Date(order.createdAt).toLocaleString()}</dd></div>
            {order.stripeSessionId && <div className="flex"><dt className="w-24 text-zinc-400">Payment</dt><dd className="text-xs truncate">{order.stripeSessionId}</dd></div>}
          </dl>
        </div>

        <div className="border border-zinc-200 rounded-xl p-6">
          <h2 className="font-medium text-ocean mb-4">Customer</h2>
          {order.user ? (
            <dl className="space-y-3 text-sm">
          ...
          {order.shipping?.name && (
            <div className="mt-4 pt-4 border-t border-zinc-100">
              <h3 className="text-xs font-medium text-zinc-400 mb-2">Shipping Address</h3>
      ...
      <div className="border border-zinc-200 rounded-xl p-6 mb-6">
        <h2 className="font-medium text-ocean mb-4">Items</h2>
        <table className="w-full text-sm">
          <thead><tr className="border-b border-zinc-200 text-left text-zinc-400">
            <th className="pb-2">Product</th><th className="pb-2">Price</th><th className="pb-2">Qty</th><th className="pb-2 text-right">Subtotal</th>
          </tr></thead>
          <tbody>
