  ...
  return (
    <header className="sticky top-0 z-50 bg-white/95 backdrop-blur border-b border-zinc-100">
      <div className="max-w-6xl mx-auto px-4 h-16 flex items-center justify-between">
        <Link href={`/${locale}`} className="text-xl font-semibold tracking-wide text-ocean shrink-0">
        ...
        <nav className="hidden md:flex items-center gap-6 text-sm font-medium text-zinc-600">
          ...
          <Link href={`/${locale}/order-tracking`} className="hover:text-gold transition-colors">Track</Link>
        </nav>
        ...
        <div className="flex items-center gap-2">
          ...
          {user ? (
            <div className="flex items-center gap-2 text-sm">
              {user.role === 'ADMIN' && <Link href={`/${locale}/admin`} className="text-gold font-medium hover:underline text-xs">Admin</Link>}
