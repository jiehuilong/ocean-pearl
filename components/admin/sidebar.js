...
const icons = { dashboard: '📊', products: '📦', orders: '📋', users: '👥' };
...
export default function AdminSidebar({ locale }) {
  ...
  const isActive = (key) => {
    const href = `/${locale}/admin${key === 'dashboard' ? '' : '/' + key}`;
    ...
    <>
      ...
      <aside className={`fixed md:static inset-y-0 left-0 z-40 w-60 bg-ocean text-zinc-300 flex flex-col transition-transform duration-200 ${open ? 'translate-x-0' : '-translate-x-full md:translate-x-0'}`}>
        ...
        <nav className="flex-1 p-3 space-y-1">
          {sections.map(key => (
            <Link key={key} href={`/${locale}/admin${key === 'dashboard' ? '' : '/' + key}`}
              onClick={() => setOpen(false)}
              className={`flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm transition-colors ${isActive(key) ? 'bg-white/10 text-white' : 'hover:bg-white/5 hover:text-white'}`}>
              <span>{icons[key]}</span> {t(key)}
            </Link>
          ))}
        </nav>
        <div className="p-3 border-t border-zinc-700">
          <Link href={`/${locale}`} className="flex items-center gap-2 px-3 py-2 text-sm hover:text-white transition-colors">
