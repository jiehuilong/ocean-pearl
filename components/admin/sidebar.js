...
export default function AdminSidebar({ locale }) {
  ...
  return (
    <>
      ...
      <aside className={`
        fixed md:static inset-y-0 left-0 z-40 w-60 bg-ocean text-zinc-300 flex flex-col transition-transform duration-200
        ${open ? 'translate-x-0' : '-translate-x-full md:translate-x-0'}
      `}>
        <div className="p-5 border-b border-zinc-700">
          <Link href={`/${locale}/admin`} className="text-white font-semibold text-lg">
          ...
          {links.map(l => (
            <Link key={l.href} href={`/${locale}${l.href}`}
              ...
              className={`flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm transition-colors ${
                isActive(l.href) ? 'bg-white/10 text-white' : 'hover:bg-white/5 hover:text-white'
              }`}>
