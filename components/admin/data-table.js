...
export default function DataTable({ columns, data, loading, emptyText, onSearch, page, total, limit = 20, searchPlaceholder }) {
  ...
  const handleSearch = useCallback((val) => {
    ...
    const tId = setTimeout(() => onSearch?.(val), 300);
    timeoutRef[1](tId);
  }, [onSearch]);
  ...
  return (
    <div>
      ...
      {loading ? (
        <div className="space-y-3">{Array.from({ length: 5 }).map((_, i) => <div key={i} className="h-12 bg-zinc-100 rounded animate-pulse" />)}</div>
      ) : data.length === 0 ? (
            ...
            <tbody>
              {data.map((row, i) => (
                <tr key={row.id || i} className="border-b border-zinc-100 hover:bg-zinc-50 transition-colors">
                  {columns.map(col => <td key={col.key} className="py-3 pr-4">{col.render ? col.render(row) : row[col.key]}</td>)}
                </tr>
              ))}
            </tbody>
          </table>
